...
ArcGisControl.ArcGisMap = function (element) {
    ArcGisControl.ArcGisMap.initializeBase(this, [element]);
...
ArcGisControl.ArcGisMap.prototype = {
    initialize: function () {
        ArcGisControl.ArcGisMap.callBaseMethod(this, 'initialize');

        // Add custom initialization here
       
        require(["esri/map", "esri/layers/ArcGISTiledMapServiceLayer", "esri/layers/GeoRSSLayer", "dojo/domReady!"],
            dojo.hitch(this, "setupMap"));
        

    },
    dispose: function () {
        //Add custom dispose actions here
        ArcGisControl.ArcGisMap.callBaseMethod(this, 'dispose');
    },
    get_zoom: function () {
        return this._zoom;
    },
    ...
    , setupMap: function (Map, Tile, Geo) {
        map = new Map(this.get_clientid());
        var tiled = new Tile("http://www.sunderland.gov.uk/LocalViewExt/ArcGIS/Rest/Services/31234cdc-e421-4ae1-8f7e-1a3a5ba75fb3/MapServer");
        map.addLayer(tiled);
        //debugger;

        if (this.get_geoRSS() != "") {
            var georss = new Geo(this.get_geoRSS(), {
                outSR: new esri.SpatialReference({ wkid: 27700 })
            });
            map.addLayer(georss);
        ...
        //dojo.connect(map, "onClick", this.get_onClick());
    }


}
ArcGisControl.ArcGisMap.registerClass('ArcGisControl.ArcGisMap', Sys.UI.Control);
